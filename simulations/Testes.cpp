...
int main() {
  ...
  cin >> r_init; */
	...
	number_of_census = 12500; grid_size =128; birth_probability =0.704; r_init =16; lag=1;

	//Taken from: http://www.cplusplus.com/forum/unices/112048/

	float data[40][7];
  std::ifstream file("dump/15_16_FuckYou.csv");
  ...
  for(int row = 0; row < 40; ++row)
  {
	...
	for(int i=0; i< 40; i++)
	{
		for(int j=1; j <7; j++)
		{
			f_coordinates temp; temp.x = data[i][0]; temp.y =data[i][j]; pL.push_back(temp);
		}
	}
	divisions = 40*6;
	cout << "p\t|  lag\n";
	...
	//find_equilibrium_single_shot_transformations_tp(grid_size, divisions, t_to_eq, how_many, transformations, pL);
